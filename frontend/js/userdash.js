...
/* ════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════ */
let userLocatorMapInstance = null;
let mapLocationsData = [];

async function initUserLocator(forceRefresh = false) {
    if (userLocatorMapInstance && !forceRefresh) return;
    
    const container = document.getElementById('userLocatorMap');
    if (!container) return;
    
    if (typeof maplibregl === 'undefined') {
        setTimeout(() => initUserLocator(forceRefresh), 500);
        return;
    }
    
    // Check if map object already exists in container and remove it
    if (userLocatorMapInstance) {
        userLocatorMapInstance.remove();
        userLocatorMapInstance = null;
    }
    
    container.innerHTML = '<div style="display:flex;height:100%;align-items:center;justify-content:center;color:white;"><i class="fas fa-spinner fa-spin" style="margin-right:10px;"></i> Loading 3D Map...</div>';
    
    try {
        const res = await fetch(`${API}/user/locations`, { credentials: 'include' });
        mapLocationsData = await res.json();
        
        container.innerHTML = ''; // Clear loading spinner
        
        // Use a 3D-capable style if possible, Carto Voyager is a good base
        userLocatorMapInstance = new maplibregl.Map({
        ...
        // Add controls
        userLocatorMapInstance.addControl(new maplibregl.NavigationControl({
            visualizePitch: true
        }), 'top-right');
        
        userLocatorMapInstance.addControl(new maplibregl.GeolocateControl({
            positionOptions: { enableHighAccuracy: true },
            trackUserLocation: true
        }), 'top-right');
        
        userLocatorMapInstance.on('load', () => {
            // Add 3D Buildings
            const layers = userLocatorMapInstance.getStyle().layers;
            let labelLayerId;
            ...
            if (Array.isArray(mapLocationsData) && mapLocationsData.length > 0) {
                const bounds = new maplibregl.LngLatBounds();
                
                mapLocationsData.forEach(loc => {
                    if (loc.lat && loc.lng) {
                        ...
                        el.innerHTML = loc.type === 'atm' ? '<i class="fas fa-money-bill-wave"></i>' : '<i class="fas fa-university"></i>';
                        
                        // Add ripple effect for branches
                        if (loc.type === 'branch') {
                            ...
                            ripple.style.border = '2px solid #800000';
                            ripple.style.animation = 'ripple 2s infinite ease-out';
                        ...
                        const popupHtml = `
                            <div style="padding:15px;text-align:center;min-width:200px;">
                                ...
                                <h4 style="margin:0 0 5px 0;font-size:16px;">${escHtml(loc.name)}</h4>
                ...
                if (!bounds.isEmpty()) {
                    userLocatorMapInstance.fitBounds(bounds, { padding: 60, maxZoom: 15 });
                    // Rotate automatically on start for a cool 3D effect
                    userLocatorMapInstance.easeTo({
                        bearing: 20,
                        duration: 8000,
                        easing: (t) => t * (2 - t)
                    });
                ...
                showToast('No branches or ATMs found.', 'warning');
            }
        });
        
    } catch (err) {
        console.error(err);
        container.innerHTML = '<div style="display:flex;height:100%;align-items:center;justify-content:center;color:#ef4444;"><i class="fas fa-exclamation-circle" style="margin-right:10px;"></i> Failed to load map data</div>';
    }
}
